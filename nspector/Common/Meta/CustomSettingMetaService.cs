...
namespace nspector.Common.Meta
{
    internal class CustomSettingMetaService : ISettingMetaService
    {
        ...
        public uint? GetDwordDefaultValue(uint settingId)
        {
            ...
            if (setting != null)
                return setting.DefaultValue.HasValue ? (uint?)setting.DefaultValue.Value : null;
        ...
        public List<SettingValue<uint>> GetDwordValues(uint settingId)
        {
            ...
            if (setting != null)
            {
                ...
                {
                    ValuePos = i++,
                    Value = (uint)x.SettingValue,
                    ValueName = _source == SettingMetaSource.CustomSettings ? x.UserfriendlyName : DrsUtil.GetDwordString((uint)x.SettingValue) + " " + x.UserfriendlyName,
                }).ToList();
            }
        ...
        public List<uint> GetSettingIds()
        {
            return customSettings.Settings
                 .Select(x => (uint)x.SettingId).ToList();
        }
