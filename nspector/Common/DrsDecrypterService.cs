...
namespace nspector.Common
{
    internal class DrsDecrypterService : DrsSettingsServiceBase
    {
        ...
        private readonly HashSet<string> _InternalSettings = new HashSet<string>();
        ...
        {
            int startIndex = (int)(offset % (_InternalSettingsKey.Length - 3));
            return BitConverter.ToUInt32(_InternalSettingsKey, startIndex);
        }
        ...
        private void CreateInternalSettingMap()
        {
            ...
            try
            {
                ...
                if (File.Exists(tmpfile))
                {
                    ...
                    var paProfile = new Regex(@"Profile\s\""(?<profileName>.*?)\""", RegexOptions.Compiled);
                    var paSetting = new Regex(@"ID_0x(?<sid>[0-9a-fA-F]+)\s\=.*?InternalSettingFlag\=V0", RegexOptions.Compiled);

                    string currentProfileName = "";
                    foreach (var line in lines)
                    {
                        var profileMatch = paProfile.Match(line);
                        if (profileMatch.Success)
                        {
                            currentProfileName = profileMatch.Groups["profileName"].Value;
                        }

                        var settingMatch = paSetting.Match(line);
                        if (settingMatch.Success)
                        {
                            _InternalSettings.Add(currentProfileName + settingMatch.Groups["sid"].Value.ToLowerInvariant());
                        }
                    }
                }
            }
